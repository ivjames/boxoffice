...
from orders.models import Hold, Order, OrderItem, Payment
from passes import services as passes_services
from passes.models import PassProduct, PassPurchase

# The vendor-agnostic fulfillment core (fulfill_hold/fulfill_donation/
# fulfill_pass_purchase/fulfill_hold_with_pass and the FulfillmentError family)
# now lives in fulfillment/services.py -- see that module's docstring. Imported
# here, rather than referenced via fulfillment.services.<name>, so (a)
# fulfill_checkout_session below can keep calling fulfill_hold/fulfill_donation/
# fulfill_pass_purchase unqualified, (b) TenantMismatchError(FulfillmentError)
# below can keep subclassing it, and (c) this RE-EXPORTS these names so existing
# `payments.services.<name>` / `from payments.services import <name>` callers
# and tests keep working unchanged. Intentional backward-compat re-export.
from fulfillment.services import (  # noqa: F401
    AvailabilityChangedError,
    DonationDataError,
    FulfillmentError,
    HoldGoneError,
    PassDataError,
