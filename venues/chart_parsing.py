...
_SECTION_SCHEMA = {
    ...
    "required": [
        ...
        "seat_pitch", "row_pitch", "arc_radius", "numbering_scheme", "seat_number_base",
...
- A whole block visibly tilted, its rows no longer horizontal = rotation in \
degrees. Don't try to capture per-row staggering/raking or how ragged rows \
are aligned -- staff set that with one slider in the editor; just get the \
seat counts (via removed_seats) and numbering right.
        ...
        sections.append(
            {
                ...
                # Per-row staggering/raking is no longer parsed from the
                # chart -- staff set offset_mode/row_x_offset/alt_row_seat_delta
                # with the editor's slider during onboarding (a far more
                # reliable one-slider fix than a fragile visual heuristic).
                # The fields stay at their Section defaults so the roster and
                # geometry are a plain grid the editor then shapes.
